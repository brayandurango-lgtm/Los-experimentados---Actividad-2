...
public class FacturaService {
    ...
    @Transactional(readOnly = true) public List<Factura> listar() {
    ...
    @Transactional(readOnly = true) public List<Pedido> pedidosSinFactura() { return pedidoRepository.findByFacturaIsNull(); }
    @Transactional(readOnly = true) public long contarPendientes() { return facturaRepository.countByEstado(Factura.Estado.PENDIENTE); }
    @Transactional(readOnly = true) @SuppressWarnings("null") public Factura buscarPorId(Long id) {
    ...
    @Transactional public Factura crear(Factura factura) {
        if (factura == null) throw new IllegalArgumentException("La factura es obligatoria");
        if (factura.getPedido() == null || facturaRepository.existsByPedidoId(factura.getPedido().getId())) {
        ...
        Pago pago = new Pago();
        pago.setFecha(guardada.getFecha());
        pago.setValor(guardada.getTotal());
        pago.setMetodoPago("Pago de factura");
        pago.setEstado(Pago.Estado.PAGADO);
        pagoService.registrar(guardada, pago);
        guardada.setEstado(Factura.Estado.PAGADA);
    ...
    @Transactional(readOnly = true) public BigDecimal totalVentasPagadas() { return pagoRepository.totalRecibido(); }
