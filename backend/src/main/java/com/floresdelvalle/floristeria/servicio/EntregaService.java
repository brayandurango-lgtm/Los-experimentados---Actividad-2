...
public class EntregaService {
    ...
    @Transactional public Entrega crear(Entrega entrega) { if (entrega == null) throw new IllegalArgumentException("La entrega es obligatoria"); entrega.setId(null); return guardar(entrega); }
    @Transactional public Entrega actualizar(Long id, Entrega datos) { Entrega entrega = buscarPorId(id); if (datos == null) throw new IllegalArgumentException("Los datos de la entrega son obligatorios"); entrega.setPedido(datos.getPedido()); entrega.setConductor(datos.getConductor()); entrega.setDireccionEntrega(datos.getDireccionEntrega()); entrega.setFechaProgramada(datos.getFechaProgramada()); entrega.setEstado(datos.getEstado()); entrega.setObservaciones(datos.getObservaciones()); return guardar(entrega); }
